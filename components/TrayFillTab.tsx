...
import { MapPin, Calculator, FileCode } from 'lucide-react';
    ...
    <div className="flex h-full bg-slate-900 text-slate-200 overflow-hidden">
      <div className="w-1/4 flex flex-col border-r border-slate-800 bg-slate-800">
        <div className="p-4 border-b border-slate-700 flex justify-between items-center">
          <button onClick={() => setSelectedNode(null)} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${!selectedNode ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>
              Global Analysis
          </button>
          <button onClick={exportToDxf} disabled={!systemResult} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-30 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-lg transition-all">
      ...
      <div className="flex-1 flex flex-col relative bg-slate-100">
        {!selectedNode && cableData.length > 0 ? (
            <div className="absolute inset-0 bg-slate-900/95 z-20 flex flex-col items-center justify-center text-center p-12 backdrop-blur-sm">
                <div className="w-20 h-20 bg-blue-900/50 rounded-full flex items-center justify-center mb-4 text-blue-400 border border-blue-500/30 shadow-[0_0_30px_rgba(59,130,246,0.3)]"><MapPin size={40}/></div>
                <h2 className="text-2xl font-bold text-white mb-2 tracking-tight">Select a Node to Visualize</h2>
                <p className="text-slate-400 max-w-sm text-sm font-medium leading-relaxed">
                    All cable paths have been aggregated. Select a specific node point from the left panel to calculate the required tray width at that specific location.
                </p>
                <div className="mt-6 flex gap-2">
                    <button onClick={() => setSelectedNode(nodeStats[0]?.name)} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest shadow-xl hover:scale-105 transition-transform">
                        Pick Busiest Node ({nodeStats[0]?.name})
                    </button>
                </div>
            </div>
        ) : null}
        ...
        {systemResult ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-600 gap-4 opacity-50 bg-slate-900">
             <Calculator size={64}/>
